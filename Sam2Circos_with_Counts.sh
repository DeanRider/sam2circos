...
# translocations. The starting and ending information and the count
...
awk -F "\t" '{if($2=="0" || $2=="16") print $3,$4,$16}' $parseref | grep -F "SA:Z:" | awk -F' |,' 'BEGIN{OFS="\t";} {print $1, $2, $2, $3, $4, $4}' | sed 's/NC_00000/chr/g ; s/NC_0000/chr/g ; s/SA:Z://g ; s/\.9//g ; s/\.10//g ; s/\.11//g ; s/\.12//g ; s/\.14//g' | sort | uniq -c | awk 'BEGIN{OFS="\t";} {print $2,$3,$4,$5,$6,$7,$1}' > $parseref.Circos.tsv || true
